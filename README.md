...
### Установка скрипта в таблицу
Если хочется сделать чтобы можно было менять категории расходов из гугл таблиц:
1. Создайте таблицу как на скрине ниже:
    <p align="center">
        <img src="./result_imgs/google_sheets/expenses_sheet" alt="Вид гугл таблицы для работы синхронизации" width="600"/>
        <br>
        <i>Гугл таблица для расходов</i>
    </p>
    <br>
    В данном примере столбец "I" скрывается, поэтому он никак не подписан, в этом 
    столбце хранятся id расходов
2. Перейдите в Расширения -> Apps Script на верхней панели:
    <p align="center">
        <img src="./result_imgs/google_sheets/navigation_to_script" alt="Навигация по гугл таблицам для создания скрипта" width="300"/>
        <br>
        <i>Навигация по гугл таблицам для создания скрипта</i>
    </p>
3. Вставьте [скрипт](./google_sheets_scripts/sync_categories.js) в редактор, по желанию можно изменить
INACTIVITY_DELAY, это настройка минимального времени от последнего действия пользователя.
4. Сохраните полученный скрипт, нажмите на "Начать развертывание" -> "Новое развертывание", желательные настройки на скрине (описание можно любое):
    <p align="center">
        <img src="./result_imgs/google_sheets/script_deploy" alt="Развертывание скрипта" width="400"/>
        <br>
        <i>Развертывание скрипта</i>
    </p>
    <br>
    После одного развертывания достаточно лишь сохранять файл, если хотите его изменить.
5. Проверка логов.  
    Чтобы посмотреть лог скрипта в левом меню выберите "Количество выполнений". Там будут расписаны все вызовы функций на триггерах, чтобы посмотреть лог конкретного выполнения достаточно лишь нажать на строчку с нужным вызовом.
6. Просмотр скрытой таблицы.  
    Бывает, что нужно просмотреть какие изменения ещё не были приняты нашим приложением, для таких ситуаций есть [скрипт](./google_sheets_scripts/show_hidden_sheet.js), он не требует развертывания, достаточно сохранить файл, выбрать функцию и выполнить. Функция hideExampleSheet скроет таблицу, а showExampleSheet покажет.
7. В процессе создания скрипта могут возникать предупреждения о том что гугл не доверяет источнику, это нормальное поведение, чтобы разрешить использование нажмите "Дополнительно", в раскрывшейся части согласитесь с доверием источнику скрипта.
...
### 06.04.25
Добавлено:  
Появилась [логика](./utils/tinkoff/sync_google_category.py), получающая измененные в гугл таблице категории + [скрипт](./google_sheets_script.js) 
для таблиц (инструкция выше).


### 25.03.25
Изменено:  
Новый вывод расходов в гугл.  
1. Теперь учитывается последняя дата, записанная в таблице, 
если за эту дату есть не добавленные расходы (или у расхода не было id),
то добавляется за ту же дату (порядок не сохраняется).
2. Обновляются категории в соответствии с теми, что были в бд


### 24.03.25
Добавлен новый [файл](./google-db_migration.py) в корне. В нем в бд выгружаются категории из гугла и в гугл айдишники из бд (в следующем обновлении изменится выгрузка в гугл).
